...
public class RecipeNutrientAnalyzer {
    ...
    private static final Cache<Identifier, AnalysisResult> ANALYSIS_CACHE =
    ...
    private static final Map<String, FoodProfile> NON_FOOD_NUTRIENTS = new HashMap<>();
    ...
    static {
        ...
        NON_FOOD_NUTRIENTS.put("wheat", FoodProfile.of(1.0f, 0, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("flour", FoodProfile.of(1.0f, 0, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("rice", FoodProfile.of(1.0f, 0, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("corn", FoodProfile.of(1.0f, 0, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("oat", FoodProfile.of(1.0f, 0, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("barley", FoodProfile.of(1.0f, 0, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("rye", FoodProfile.of(1.0f, 0, 0, 0, 0));

        // Proteins
        NON_FOOD_NUTRIENTS.put("egg", FoodProfile.of(0, 1.0f, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("butter", FoodProfile.of(0, 0.5f, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("cheese", FoodProfile.of(0, 0.8f, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("cream", FoodProfile.of(0, 0.3f, 0, 0, 0));

        // Sugars
        NON_FOOD_NUTRIENTS.put("sugar", FoodProfile.of(0, 0, 0, 0, 1.0f));
        NON_FOOD_NUTRIENTS.put("honey", FoodProfile.of(0, 0, 0, 0, 1.0f));
        NON_FOOD_NUTRIENTS.put("syrup", FoodProfile.of(0, 0, 0, 0, 1.0f));
        NON_FOOD_NUTRIENTS.put("chocolate", FoodProfile.of(0, 0, 0, 0, 0.8f));
        NON_FOOD_NUTRIENTS.put("cocoa", FoodProfile.of(0, 0, 0, 0, 0.5f));

        // Liquids (these SHOULD have water)
        NON_FOOD_NUTRIENTS.put("milk", FoodProfile.of(0, 0.2f, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("broth", FoodProfile.of(0, 0.1f, 0, 0, 0));
        NON_FOOD_NUTRIENTS.put("stock", FoodProfile.of(0, 0.1f, 0, 0, 0));
     ...
     */
    public static FoodProfile analyze(Item item, FoodSystemSettings settings) {
     ...
     */
    private static AnalysisResult analyzeRecursive(Item item, Set<Item> visitedItems, int depth, FoodSystemSettings settings) {
        Identifier itemId = BuiltInRegistries.ITEM.getKey(item);
        // Safety check: prevent infinite recursion
        if (depth > MAX_SAFETY_DEPTH) {
            LOGGER.warn("[DIET] Max recursion depth reached for {}", BuiltInRegistries.ITEM.getKey(item));
            return new AnalysisResult(FoodProfile.EMPTY, AnalysisMethod.FAILED);
        }

        // Prevent circular dependencies
        if (visitedItems.contains(item)) {
            return new AnalysisResult(FoodProfile.EMPTY, AnalysisMethod.CIRCULAR);
        }
        ...
        FoodProfile nonFoodProfile = getNonFoodNutrients(item);
        ...
        if (server != null) {
            ...
            FoodProfile recipeProfile = analyzeFromRecipe(item, recipeManager, visitedItems, depth, settings);
        ...
        if (isFood) {
            FoodProfile heuristicProfile = analyzeFromHeuristics(item, food, settings);
        ...
        return new AnalysisResult(FoodProfile.EMPTY, AnalysisMethod.NOT_FOOD);
    }
     ...
     */
    private static FoodProfile analyzeFromRecipe(Item item, RecipeManager recipeManager, Set<Item> visitedItems, int depth, FoodSystemSettings settings) {
        Identifier itemId = BuiltInRegistries.ITEM.getKey(item);
        ...
        for (RecipeHolder<?> recipeHolder : matchingRecipes) {
            ...
            for (Ingredient ingredient : ingredients) {
                ...
                LOGGER.info("[DIET DEBUG]     Result: {} (method: {})",
                        ingredientResult.profile.isEmpty() ? "EMPTY" : "grain=" + ingredientResult.profile.get(FoodCategories.GRAIN),
            ...
            FoodProfile profile = combineComponentNutrients(componentNutrients, resultCount);

            LOGGER.info("[DIET DEBUG]   Combined profile before rounding: grain={}", profile.get(FoodCategories.GRAIN));

            // Apply rounding: values < 0.1 become 0.0
            profile = roundProfile(profile);

            LOGGER.info("[DIET DEBUG]   After rounding: grain={}", profile.get(FoodCategories.GRAIN));
        ...
        if (bestProfile != null) {
            LOGGER.info("[DIET DEBUG] Final profile for {}: grain={}", itemId, bestProfile.get(FoodCategories.GRAIN));
        }
        return bestProfile;
    }
     ...
     */
    private static FoodProfile combineComponentNutrients(List<ComponentNutrients> components, int resultCount) {
        float totalGrain = 0, totalProtein = 0, totalVegetable = 0, totalFruit = 0, totalSugar = 0;

        for (ComponentNutrients component : components) {
            FoodProfile profile = component.profile;
            int count = component.count;

            totalGrain += profile.get(FoodCategories.GRAIN) * count;
            totalProtein += profile.get(FoodCategories.PROTEIN) * count;
            totalVegetable += profile.get(FoodCategories.VEGETABLE) * count;
            totalFruit += profile.get(FoodCategories.FRUIT) * count;
            totalSugar += profile.get(FoodCategories.SUGAR) * count;
        }
     ...
     */
    private static FoodProfile getNonFoodNutrients(Item item) {
        Identifier itemId = BuiltInRegistries.ITEM.getKey(item);
        ...
        for (Map.Entry<String, FoodProfile> entry : NON_FOOD_NUTRIENTS.entrySet()) {
     ...
     */
    public static FoodProfile analyzeFromHeuristics(Item item, FoodProperties food, FoodSystemSettings settings) {
        ...
        boolean meatLike = ModHeuristics.isMeatLike(item);
        boolean vegetableHint = ModHeuristics.hasVegetableHints(item);
        boolean fruitHint = ModHeuristics.hasFruitHints(item);
        boolean grainHint = ModHeuristics.hasGrainHints(item);
        ...
        FoodProfile baseProfile = FoodProfile.of(
        ...
        Identifier itemId = BuiltInRegistries.ITEM.getKey(item);
        return ModHeuristics.applyModHeuristics(itemId, baseProfile);
    }
    ...
    private static ItemStack safeGetRecipeResult(Recipe<?> recipe) {
        try {
            ...
            if (!displays.isEmpty()) {
                ContextMap context = currentServer != null
                        ? new ContextMap.Builder()
                        .withParameter(SlotDisplayContext.REGISTRIES, currentServer.registryAccess())
                        .create(SlotDisplayContext.CONTEXT)
                        : ContextMap.EMPTY;
                ItemStack result = displays.getFirst().result().resolveForFirstStack(context);
                if (!result.isEmpty()) {
                    return result;
                }
            }
     ...
     */
    private static FoodProfile roundProfile(FoodProfile profile) {
        float grain = profile.get(FoodCategories.GRAIN);
        float protein = profile.get(FoodCategories.PROTEIN);
        float vegetable = profile.get(FoodCategories.VEGETABLE);
        float fruit = profile.get(FoodCategories.FRUIT);
        float sugar = profile.get(FoodCategories.SUGAR);
     ...
     */
    private static record ComponentNutrients(FoodProfile profile, int count) {}

    /**
     * Result of nutrient analysis with method used.
     */
    private static record AnalysisResult(FoodProfile profile, AnalysisMethod method) {}
